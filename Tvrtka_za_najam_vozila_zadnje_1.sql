...
-- 2.   PRIKAZ 5 VRSTA DODATNE OPREMU KOJA SE NAJMANJE PUTA IZNAJMILA
...
-- 3.  PROVJERA NAJMA VOZILA KOJI CU PREMAŠILI ODREĐENI BROJ KILOMETARA
...
-- 5. PRONALAŽENJE NAJMA VOZILA PO DRŽAVI SJEDIŠTA PRAVNE OSOBE
...
-- 6.  PRIKAZ TROŠKOVA NAJMA VOZILA PO ZAPOSLENIKU
...
-- 7.Prikaži prosječno trajanje najma vozila za svakog klijenta koji je unajmio vozilo u 2024. godini. 
-- Uključi i broj najma i ukupni prihod za svakog klijenta u 2024. godini.
SELECT klijent.ime, klijent.prezime, vozilo.registracijska_tablica,  serija.ime AS model_vozila,
    AVG(DATEDIFF(najam_vozila.datum_zavrsetka, najam_vozila.datum_pocetka)) AS prosjecno_trajanje_najma,
    COUNT(najam_vozila.id) AS broj_najma,
    SUM(transakcija.iznos) AS ukupni_prihod
FROM najam_vozila
INNER JOIN  transakcija ON najam_vozila.id_transakcija_najam = transakcija.id
INNER JOIN  klijent ON transakcija.id_klijent = klijent.id
INNER JOIN vozilo ON najam_vozila.id_vozilo = vozilo.id
INNER JOIN serija ON vozilo.id_serija = serija.id
WHERE najam_vozila.status = 'Neaktivan' 
    AND YEAR(najam_vozila.datum_pocetka) = 2024
GROUP BY klijent.ime, klijent.prezime,  vozilo.registracijska_tablica, serija.ime
...
SELECT 
    v.id AS 'ID vozila',
    s.ime AS 'Naziv serije',
    v.tip_punjenja AS 'Tip punjenja',
    ...
    (
        SELECT 
            GROUP_CONCAT(DISTINCT datum_pocetka SEPARATOR ', ')
        FROM 
            najam_vozila
        WHERE 
            id_vozilo = v.id
    ...
    (
        SELECT 
            GROUP_CONCAT(DISTINCT datum_zavrsetka SEPARATOR ', ')
        FROM 
            najam_vozila
        WHERE 
            id_vozilo = v.id
    ...
    (
        SELECT 
            SUM(DATEDIFF(datum_zavrsetka, datum_pocetka))
        FROM 
            najam_vozila
        WHERE 
            id_vozilo = v.id
...
JOIN 
    serija s ON v.id_serija = s.id
LEFT JOIN 
    najam_vozila nv ON v.id = nv.id_vozilo
WHERE 
    v.tip_punjenja = 'Električno'
GROUP BY 
    v.id, s.ime;
...
-- UPIT 6
-- Prikaži svva plaćanja izvršena kriptovalutnim načinom plaćanja, datum kad su izvršena i od strane kojeg klijenta.
